function [DI, resps, t_acc_indiv] = DA_get_dir_F(data)
% Calclating the Direction Index for each digit for a given dataset. 
%
% This is for the non-dominant foot datasets (toes only)
%
% 130417 (NC)

t_idx_1 = find(data(:,2) == 1);
t_idx_2 = find(data(:,2) == 2);
t_idx_3 = find(data(:,2) == 3);
t_idx_4 = find(data(:,2) == 4);
t_idx_5 = find(data(:,2) == 5);


% (1) ACCURACY OF EACH TOE RESPONSE:
...
for i = 1:5
    ...
    resps(i,1) = (size(find(data(t_idx_1, 3) == i),1));
end

for i = 1:5
    
    resps(i,2) = (size(find(data(t_idx_2, 3) == i),1));
end

for i = 1:5
    
    resps(i,3) = (size(find(data(t_idx_3, 3) == i),1));
end

for i = 1:5
    
    resps(i,4) = (size(find(data(t_idx_4, 3) == i),1));
end

for i = 1:5
    
    resps(i,5) = (size(find(data(t_idx_5, 3) == i),1));
end
